...
>
> **Phase 0 — Archivage & baseline (✅ terminée, étiquette `v17-documents-baseline`)**
> - Les 7 générateurs actuels copiés en lecture seule dans `src/lib/documents/legacy/*.v1.tsx` (certificate, reminder, info-sheet, timetable, receipt, invoice, report-card) — voir la section « Point de Sauvegarde » dédiée ci-dessous pour le détail et la commande de retour.
> - Baseline PDF par rendu serveur réel : `scripts/generate-documents-baseline.ts` pilote Chromium headless par CDP et appelle `Page.printToPDF` (le moteur natif de `window.print()`, aucune brique de fidélité ajoutée). 6 PDF dans `docs/documents-reference/` + `manifest.json`, depuis des données RÉELLES de SENG.CO ACADEMY.
> - Connexion pour la capture : `sessionCookiesForExistingUser()` (nouveau, additif dans `scripts/_cdp.ts`) mine un jeton magic-link via l'API Admin Supabase pour se connecter en lecture seule sous une identité RÉELLE (OWNER `koryobjectifs@gmail.com`) — **sans mot de passe, sans e-mail envoyé, zéro écriture en base**. ⚠️ `verifyOtp` refuse `email` + `token_hash` ensemble (« Only the token_hash and type should be provided ») : ne passer QUE `token_hash` + `type`.
> - **Lettre de relance SKIPPÉE** : 0 facture en retard sur SENG.CO au moment de la capture. Baseline sur écran vide jugée inutile à la comparaison (décision Kory) — **aucune facture fabriquée** pour combler le trou. Consignée dans `manifest.json → skipped[]` ; relancer le même script quand une vraie facture OVERDUE existera la capturera automatiquement, sans rien changer au code.
> - 4 suites + typecheck après la phase : `tsc --noEmit` 0 erreur, navigation 55/55, unconfirmed-block 21/21, permissions-matrix 43/43. **`verify-design-tokens` : 1 échec, PRÉ-EXISTANT, sans rapport avec ce chantier** — voir piège ci-dessous.
>
> ⚠️ **Piège trouvé en marge — `verify-design-tokens.ts` teste un contrat de thème périmé depuis le lot 7 du chantier Shell (10 septembre).** Le test (assertion vers la ligne 168) attend que `schoolThemeStyle("#7C2D12")` surcharge `--color-primary`. Or `src/lib/theme.ts` a été réécrit à ce lot-là (« Isolation absolue des boutons ») : la fonction n'émet plus JAMAIS `--color-primary`, seulement `--color-frame-bg/topbar-bg/rail-bg/sidebar-*` — précisément pour que la couleur d'école n'atteigne plus les boutons. Le test n'a pas suivi le changement de contrat. Échec systématique depuis, indépendant de tout travail sur les documents. À corriger dans un futur lot Shell : mettre à jour l'assertion du test, surtout pas le code de `theme.ts` qui est le comportement voulu.
>
> **Phase 1 — Audit (✅ livré à Kory) — trois pièges de DONNÉES trouvés en cours d'audit, hors périmètre du générateur, chantiers à part :**
>
> ⚠️ **Bulletin (SENG.CO / classe CE2, 40 élèves inscrits) : une seule élève a des notes saisies au 1er Trimestre — Ndeye Diallo, 32 notes.** Les 39 autres n'en ont aucune (vérifié : `Grade.groupBy` sur la classe → 1 seul `studentId` distinct). `buildBulletin()` (`src/lib/bulletin.ts`) est vérifié **correct** : moyenne de classe, extrêmes et rang par matière se calculent sur l'ensemble des élèves réellement notés ; avec un seul élève noté, ils valent trivialement sa propre note (d'où « moyenne classe = note élève » et « extrêmes 15.90/15.90 »), et son rang est 1 sur 1 classé — affiché « / 40 » car ce dénominateur vient de l'effectif réel (`bulletin.headcount`, une source différente et correcte), d'où l'incohérence apparente. **Ce n'est pas un bug de calcul, c'est un manque de données.** Les appréciations identiques (« Tres bon travail ! », sans accent) sont la MÊME valeur stockée telle quelle sur 11 des notes de cette élève (pas une recopie d'écran) — aucune trace dans les scripts de seed (grep négatif) : très probablement saisie une fois à la main pendant un test manuel de l'écran de saisie, jamais une donnée réelle. Les 3 lignes sans appréciation (IST, Dessin/Arts plastiques, EPS) ont un `Grade.comment` réellement `NULL` — comportement voulu du gabarit (jamais inventer un texte), pas un défaut d'affichage. **Chantier à part (Kory) : ressaisir des notes réalistes sur les 39 autres élèves de CE2 avant toute démo du bulletin.**
>
> ⚠️ **Facture / reçu : trois implémentations indépendantes du même document, jamais unifiées.** `payments/invoice/` et `payments/receipt/` (les écrans que le générateur documents expose) n'ont AUCUN `actions.ts` et ne touchent jamais `Invoice`/`Payment` — composeurs de brouillon 100 % client (`localStorage`), numéro et montants inventés (`FAC-2026-xxxx`, 45 000 F, « Frais de Cantine » à 15 000 F par défaut). La VRAIE création vit dans `payments/new/form.tsx` → `payments/actions.ts` (`createInvoice`), avec SA PROPRE UI d'impression ; et une TROISIÈME, `payments/[id]/InvoiceViewerClient.tsx`, pour revisualiser une facture déjà émise. Trois rendus du même document officiel, comme le bulletin avant son unification — jamais fait ici. **Chantier à part (décision Kory) : ne sera pas corrigé dans le générateur de documents actuel** ; effort estimé de reconnexion aux vraies données détaillé dans l'audit remis à Kory.
>
> **Constat transversal de l'audit** : sur les 7 générateurs, seul le bulletin (`BulletinSheet`) injecte `school.logo/signature/stamp` — les 6 autres l'ignorent (le certificat fait dessiner une signature ad hoc à chaque impression via `SignaturePad`, jamais la signature enregistrée de l'école). `School.primaryColor` n'est utilisé dans AUCUN document (confirmé par grep exhaustif) — seulement dans le chrome de l'application (`dashboard/layout.tsx`). Sur SENG.CO : logo et signature présents, cachet absent. Le bulletin de référence (`docs/documents-reference/bulletin-de-notes.pdf`) ne montre pourtant ni l'un ni l'autre malgré le câblage correct dans `BulletinSheet.tsx` — hypothèse la plus probable, **non vérifiée visuellement** : le bloc visa est en fin d'un conteneur `print:overflow-hidden` à hauteur fixée (297mm), et pourrait être rogné si le contenu du tableau de notes (16 matières sur cette classe) approche la limite de page. À vérifier à l'œil avant la phase 2.
>
> **Chantiers ouverts ajoutés par cette phase (hors périmètre du générateur de documents, à trancher par Kory) :**
> - Ressaisir des notes réalistes sur toute la classe CE2 (SENG.CO) avant toute démo de bulletin.
> - Unifier facture / reçu / visualiseur sur les vraies données `Invoice`/`Payment` (3 implémentations aujourd'hui) — quelques heures par écran, détail dans l'audit phase 1.
> - Mettre à jour `verify-design-tokens.ts` pour refléter le contrat de thème du lot 7 (ne plus attendre `--color-primary` surchargé par la couleur d'école).
> - Vérifier à l'œil si le cachet/la signature du bulletin sont réellement rognés par débordement de page, avant de bâtir la phase 2 dessus.

## 🏷️ Point de Sauvegarde & Rollback : `v17-documents-baseline`

- **Nom de l'étiquette Git** : `v17-documents-baseline`
- **Commande de Rollback / Restauration** : `git checkout v17-documents-baseline`
- **Contenu du jalon** — aucun changement de comportement applicatif, uniquement de l'archive et de l'outillage :
  1. **Archive figée** des 7 générateurs actuels, copiés (pas déplacés — les originaux restent actifs) dans `src/lib/documents/legacy/` :
     `certificate.Generator.v1.tsx`, `reminder.Generator.v1.tsx`, `info-sheet.Generator.v1.tsx`, `timetable.Generator.v1.tsx`, `receipt.Generator.v1.tsx`, `invoice.Generator.v1.tsx`, `report-card.Generator.v1.tsx`.
  2. **Baseline PDF de référence** dans `docs/documents-reference/` : `certificat-de-scolarite.pdf`, `fiche-de-renseignements.pdf`, `facture.pdf`, `recu-de-paiement.pdf`, `bulletin-de-notes.pdf`, `emploi-du-temps.pdf`, plus `manifest.json` (cibles exactes, URLs, notes, et la lettre de relance skippée avec sa raison).
  3. **Outillage réutilisable** pour régénérer la baseline plus tard (jalon B, non-régression visuelle) : `scripts/generate-documents-baseline.ts`, et deux ajouts additifs dans `scripts/_cdp.ts` (`sessionCookiesForExistingUser`, `printPDF`) — rien de supprimé ni modifié dans le comportement existant du fichier.
- **Ce que ce point NE couvre PAS** : aucune correction. Les 3 pièges de données ci-dessus (bulletin CE2, facture/reçu déconnectés, test de thème périmé) sont documentés, pas réparés.

## Chantier Précédent : TopBar Unifiée & Rail Compact (`v15-topbar-unifiee`, 10 septembre)
