...
export async function injectDemoData() {
  ...
  const classes = await Promise.all([
    prisma.class.create({ data: { name: `[DÉMO] 6ème A${DEMO_MARKER}`, cycle: "MOYEN", schoolId, isDemo: true } }),
    prisma.class.create({ data: { name: `[DÉMO] 5ème B${DEMO_MARKER}`, cycle: "MOYEN", schoolId, isDemo: true } }),
    prisma.class.create({ data: { name: `[DÉMO] 4ème C${DEMO_MARKER}`, cycle: "MOYEN", schoolId, isDemo: true } })
  ]);
