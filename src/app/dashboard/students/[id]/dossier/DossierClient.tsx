...
import {
  ...
  ArrowLeft, ChevronDown, Plus, FolderPlus, PenTool, Eye, CheckCircle2, Camera,
  UploadCloud,
} from "lucide-react";
 ...
 * ou « fichiers » est ce qui rend l'écran utilisable sur téléphone.
 */
...
type Line = {
  ...
  actionPrompt: string;
  actionLabel: string;
  actionType: "UPLOAD" | "SIGNATURE" | "AUTO";
  document: {
    id: string;
    fileName: string;
    mimeType: string;
...
export function DossierClient({
  ...
  restricted, notice, events, channels, categories, folders, initialAction, initialReqId,
}: {
  studentId: string;
  student: {
  ...
  const [signReq, setSignReq] = useState<Line | null>(null);

  // ── Dépôt direct ciblé (Action directe depuis rappel parent) ──
  const [directDepositReq, setDirectDepositReq] = useState<Line | null>(null);
  const [directDepositFile, setDirectDepositFile] = useState<File | null>(null);
  ...
  useEffect(() => {
    ...
    const targetLine = lines.find((l) => l.requirementId === initialReqId);
    if (!targetLine) return;

    if (initialAction === "sign" || targetLine.actionType === "SIGNATURE") {
      setSignReq(targetLine);
    } else if (initialAction === "deposit" || targetLine.actionType === "UPLOAD") {
      setDirectDepositReq(targetLine);
    }
  }, [initialAction, initialReqId, lines]);

  const aSigner = useMemo(() => {
    return lines.filter((l) => l.actionType === "SIGNATURE" && l.status !== "VALIDATED");
  }, [lines]);

  const aDeposer = useMemo(() => {
    return lines.filter((l) => l.actionType === "UPLOAD" && l.status !== "VALIDATED");
  }, [lines]);

  const fourniParEcole = useMemo(() => {
    return lines.filter((l) => l.autoGenerated || l.actionType === "AUTO");
  }, [lines]);
  ...
  const ligneExigee = (l: Line) => (
    <li key={l.requirementId} className="rounded-control border border-rule px-3 py-2.5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {l.nature === "AUTO" && (
          <Badge size="sm" variant={l.autoGenerated ? "success" : "neutral"}>
            {l.autoGenerated ? "Automatique" : "Auto (En attente)"}
          </Badge>
        )}
        {l.nature === "SIGNATURE" && (
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          {l.autoGenerated && l.autoLink && (
            <Link href={l.autoLink} className={LIEN_SECONDAIRE}>
          ...
          {!l.autoGenerated && (
            <>
              {l.actionType === "SIGNATURE" ? (
                <Button
                  size="sm"
                  variant={l.document ? "secondary" : "primary"}
                  onClick={() => setSignReq(l)}
                >
                  <PenTool aria-hidden="true" className="h-4 w-4" />
                  {l.document ? "Signer à nouveau" : (l.actionLabel || "Lire et signer")}
                </Button>
              ) : (
                <>
                  <input
                  ...
                  >
                    ...
                    {l.document ? "Remplacer" : (l.actionLabel || "Déposer")}
                  </Button>
                </>
              )}
            </>
      ...
      {l.autoGenerated ? (
        <p className="mt-1.5 text-role-meta text-emerald-700 dark:text-emerald-400 font-medium">
          ✓ {l.autoReason || "Généré automatiquement par l'école"}
        </p>
      ) : l.autoReason ? (
        <p className="mt-1.5 text-role-meta text-amber-700 dark:text-amber-400">
          ⚠️ {l.autoReason}
        </p>
      ) : null}
    ...
    <div className="space-y-6">
      ...
      {dossierOuvert ? (
        ...
        <>
          ...
          {/* ───────────── ACTIONS QUI POUSSENT À AGIR (Point 4) ───────────── */}
          {completeness.configured && (
            <Card
              title="Actions pour le dossier"
              description="Chaque document dispose d'une action directe : à signer, à déposer ou fourni automatiquement."
              actions={
                <div className="flex flex-wrap items-center gap-2">
                  {aSigner.length > 0 && (
                    <Badge variant="warning" size="sm">
                      {aSigner.length} à signer
                    </Badge>
                  )}
                  {aDeposer.length > 0 && (
                    <Badge variant="warning" size="sm">
                      {aDeposer.length} à déposer
                    </Badge>
                  )}
                  {fourniParEcole.length > 0 && (
                    <Badge variant="success" size="sm">
                      {fourniParEcole.length} école
                    </Badge>
                  )}
                </div>
              }
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between pb-2 border-b border-rule">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-role-body font-bold text-text">À signer</span>
                    </div>
                    <span className="text-role-meta text-text-soft font-semibold">{aSigner.length}</span>
                  </div>
                  {aSigner.length === 0 ? (
                    <p className="text-role-meta text-text-soft italic py-2">Aucun document en attente de signature.</p>
                  ) : (
                    <ul className="space-y-2.5">
                      {aSigner.map((l) => (
                        <li key={l.requirementId} className="rounded-control border border-rule bg-surface p-3 space-y-2 shadow-2xs">
                          <div>
                            <p className="text-role-body font-semibold text-text leading-snug">{l.actionPrompt}</p>
                            <p className="text-role-meta text-text-soft">{l.label} · {l.required ? "Obligatoire" : "Optionnel"}</p>
                          </div>
                          <Button size="sm" variant="primary" className="w-full justify-center" onClick={() => setSignReq(l)}>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between pb-2 border-b border-rule">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-role-body font-bold text-text">À déposer</span>
                    </div>
                    <span className="text-role-meta text-text-soft font-semibold">{aDeposer.length}</span>
                  </div>
                  {aDeposer.length === 0 ? (
                    <p className="text-role-meta text-text-soft italic py-2">Toutes les pièces requises ont été déposées.</p>
                  ) : (
                    <ul className="space-y-2.5">
                      {aDeposer.map((l) => (
                        <li key={l.requirementId} className="rounded-control border border-rule bg-surface p-3 space-y-2 shadow-2xs">
                          ...
                          />
                          <input
                            ref={(el) => { cameraInputs.current[l.requirementId] = el; }}
                            type="file"
                            accept="image/*"
                            capture="environment"
                            className="sr-only"
                          ...
                          />
                          <div className="flex items-center gap-2">
                            <Button size="sm" variant="primary" className="flex-1 justify-center" onClick={() => inputs.current[l.requirementId]?.click()}>
                              <Upload className="h-3.5 w-3.5" />
                              Fichier
                            </Button>
                            <Button size="sm" variant="secondary" className="flex-1 justify-center" onClick={() => cameraInputs.current[l.requirementId]?.click()}>
                ...
                {/* 3. FOURNI PAR L'ÉCOLE */}
                <div className="space-y-3">
                  <div className="flex items-center justify-between pb-2 border-b border-rule">
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                      <span className="text-role-body font-bold text-text">Fourni par l&apos;école</span>
                    </div>
                    <span className="text-role-meta text-text-soft font-semibold">{fourniParEcole.length}</span>
                  </div>
                  {fourniParEcole.length === 0 ? (
                    <p className="text-role-meta text-text-soft italic py-2">Aucun document produit directement par l&apos;école.</p>
                  ) : (
                    <ul className="space-y-2.5">
                      {fourniParEcole.map((l) => (
                        <li key={l.requirementId} className="rounded-control border border-emerald-500/20 bg-emerald-50/30 dark:bg-emerald-950/10 p-3 space-y-2 shadow-2xs">
                          <div>
                            <div className="flex items-center gap-1.5">
                              <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600 shrink-0" />
                              <p className="text-role-body font-semibold text-text leading-snug">{l.label}</p>
                            </div>
                            <p className="text-role-meta text-emerald-700 dark:text-emerald-400 mt-0.5">
                              {l.autoReason || "Généré automatiquement par l'école"}
                            </p>
                          </div>
                          {l.autoLink && (
                            <Link href={l.autoLink} className={`${LIEN_SECONDAIRE} w-full justify-center`}>
      ...
      <SignatureDialog
        open={Boolean(signReq)}
        onClose={() => setSignReq(null)}
        studentId={studentId}
        studentName={`${student.firstName} ${student.lastName}`}
        studentClass={enrollments[0]?.className ?? null}
        requirement={signReq ? { id: signReq.requirementId, label: signReq.label, category: signReq.category } : null}
        onSigned={() => {
          setSignReq(null);
          start(() => {
      ...
      {/* ── MODALE ACTION DIRECTE RELANCE PARENT (DÉPÔT IMMÉDIAT) ── */}
      {directDepositReq && (
        <Modal
          open={true}
          onClose={() => {
            setDirectDepositReq(null);
            setDirectDepositFile(null);
          }}
          title={`Déposer : ${directDepositReq.label}`}
        >
          <div className="space-y-4">
            <p className="text-xs text-text-soft">
              Pour <span className="font-semibold text-text">{student.firstName} {student.lastName}</span>.
              Prenez une photo nette du document ou choisissez un fichier depuis votre appareil.
            </p>
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full min-h-[48px] p-3 rounded-xl border border-emerald-200 bg-emerald-50/50 hover:bg-emerald-100/50 flex items-center gap-3 text-left transition-colors"
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-800">
                  <Camera className="h-5 w-5" />
                </div>
                <div>
                  <p className="text-xs font-semibold text-emerald-950">Prendre une photo (Appareil photo)</p>
                  <p className="text-[11px] text-emerald-700">Idéal sur smartphone · cadrage direct</p>
                </div>
              </button>
              ...
              <button
                ...
                className="w-full min-h-[48px] p-3 rounded-xl border border-rule bg-surface hover:bg-sunk flex items-center gap-3 text-left transition-colors"
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
                  <UploadCloud className="h-5 w-5" />
                </div>
                <div>
                  <p className="text-xs font-semibold text-text">Choisir un fichier</p>
                  <p className="text-[11px] text-text-soft">PDF, JPG, PNG, WEBP (max. 10 Mo)</p>
                </div>
              </button>
            </div>
            ...
            {directDepositFile && (
              <div className="p-3.5 rounded-xl bg-surface border border-rule space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 min-w-0">
                    <FileText className="h-4 w-4 text-primary shrink-0" />
                    <span className="text-xs font-semibold text-text truncate">
                      {directDepositFile.name}
                    </span>
                  </div>
                  <span className="text-[11px] text-text-soft shrink-0">
                    {formatSize(directDepositFile.size)}
                  </span>
                </div>
                ...
                <Button
                  ...
                  loading={pending}
                  onClick={() => {
                    if (!directDepositFile || !directDepositReq) return;
                    send(directDepositFile, {
                      requirementId: directDepositReq.requirementId,
                      label: directDepositReq.label,
                      category: directDepositReq.category,
                    });
                    setDirectDepositReq(null);
                    setDirectDepositFile(null);
                  }}
                >
                  ...
                  <span>Confirmer et envoyer le document</span>
                </Button>
              </div>
            )}
          </div>
